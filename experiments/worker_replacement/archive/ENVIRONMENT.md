...
```bash
uv run python -m experiments.worker_replacement.run \
...
```bash
uv run --group viewer python -m experiments.worker_replacement.viewer
```

It discovers `run.json` files below `experiments/worker_replacement/outputs` and exposes

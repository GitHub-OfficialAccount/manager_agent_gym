...
TASK_BUNDLE_RENDERER_SPEC = """Manager-facing projection of atomic-lines-v2.
Group entries losslessly by attributed worker and task, then by source pointer.
Preserve every atomic evidence ID, first-observed timestep, source pointer, and
verbatim fact. Do not summarize, rank, omit, compare, or infer. Internal ledger
storage and all relation grounding remain atomic-lines-v2.
"""
TASK_BUNDLE_RENDERER_VERSION = "task-bundles-v1"
TASK_BUNDLE_RENDERER_SPEC_SHA256 = hashlib.sha256(
    TASK_BUNDLE_RENDERER_SPEC.encode("utf-8")
).hexdigest()
...
def render_task_evidence_bundles(entries: list[AtomicEvidenceEntry]) -> str:
    """Render a lossless task-grouped projection of the append-only ledger."""
    bundles: dict[tuple[str | None, str | None], dict[str, Any]] = {}
    source_blocks: dict[
        tuple[str | None, str | None], dict[str, dict[str, Any]]
    ] = {}
    for entry in entries:
        key = (entry.worker, entry.task)
        if key not in bundles:
            ...
            source_blocks[key] = {}
        sources = source_blocks[key]
        if entry.source_pointer not in sources:
            block = {"source": entry.source_pointer, "facts": []}
            sources[entry.source_pointer] = block
            bundles[key]["sources"].append(block)
        sources[entry.source_pointer]["facts"].append(
            {
    ...
    return json.dumps(
        {
            "format": TASK_BUNDLE_RENDERER_VERSION,
            "task_evidence": list(bundles.values()),
        },
        separators=(",", ":"),
        ensure_ascii=False,
    )
...
# The board now renders EXECUTION vocabulary ("done", "not started (...)") rather
# than the raw TaskStatus value (L1). This aid reasons in the raw vocabulary and
# keys terminal-state detection off "completed"/"failed", so it normalises back
# rather than being rewritten: an aid that silently stopped recognising completion
# would degrade quietly, which is the exact failure shape this project keeps
# finding. Both vocabularies map, so bundles and fixtures written before L1 still
# parse identically.
_BOARD_STATUS_TO_RAW: dict[str, str] = {
    "done": "completed",
    "running": "running",
    "failed": "failed",
    "unknown": "unknown",
    "not started (waiting on dependencies)": "pending",
    "not started (dependencies met, not yet running)": "ready",
}


def _normalise_status(rendered: str) -> str:
    """Map a rendered board state back to the raw TaskStatus vocabulary."""
    return _BOARD_STATUS_TO_RAW.get(rendered, rendered)
    ...
    while index < len(lines):
        ...
        if current_task is not None:
            ...
            if stripped.startswith("Status:"):
                current_task.status = _normalise_status(
...
class AtomicEvidenceLedgerObservationAid:
    ...
    def diagnostics(self) -> dict[str, Any]:
        return {
            ...
            "renderer_version": TASK_BUNDLE_RENDERER_VERSION,
            "renderer_spec_sha256": TASK_BUNDLE_RENDERER_SPEC_SHA256,
    ...
    def _render(self) -> str:
        rendered = render_task_evidence_bundles(self._ledger)
        self._max_rendered_chars = max(self._max_rendered_chars, len(rendered))
        return rendered

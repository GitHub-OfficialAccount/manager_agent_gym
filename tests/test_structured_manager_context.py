...
async def test_generic_summary_runs_only_when_selected() -> None:
    ...
    for arm3_mode in ("arm3i_noq", "arm3i_q", "arm3t"):
        _, arm3_builder, arm3_observation = await _run_manager_step_with_aid(
            arm3_mode
        )
        assert len(arm3_builder.calls) == 1
        assert arm3_observation.observation_aid == (
            "neutral visible-evidence summary"
        )

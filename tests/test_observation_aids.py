...
async def test_atomic_ledger_enumerates_all_eligible_lines_without_llm_or_cap(
    ...
    bundles = payload["task_evidence"]
    rendered_facts = [
        fact
        for bundle in bundles
        for source_block in bundle["sources"]
        for fact in source_block["facts"]
    ]

    assert payload["format"] == TASK_BUNDLE_RENDERER_VERSION
    assert len(bundles) == 1
    assert bundles[0]["worker"] == "portfolio_analyst"
    assert len(rendered_facts) == 14  # status + description + twelve preview lines
    ...
    assert diagnostics["renderer_version"] == TASK_BUNDLE_RENDERER_VERSION
    assert diagnostics["renderer_spec_sha256"] == TASK_BUNDLE_RENDERER_SPEC_SHA256


def test_task_bundle_rendering_is_lossless_and_smaller_than_flat_projection() -> None:
    entries = [
        AtomicEvidenceEntry(
            evidence_id="e1",
    ...
    rendered = render_task_evidence_bundles(entries)
    payload = json.loads(rendered)
    reconstructed = []
    for bundle in payload["task_evidence"]:
        for source_block in bundle["sources"]:
            for fact in source_block["facts"]:
                reconstructed.append(
                    {
                        "evidence_id": fact["id"],
                        "first_observed_timestep": fact["t"],
                        "worker": bundle["worker"],
                        "task": bundle["task"],
                        "fact": fact["text"],
                        "source_pointer": source_block["source"],
                    }
                )
    flat = json.dumps(
        [
    ...
    assert reconstructed == [entry.model_dump(mode="json") for entry in entries]
    assert len(rendered) < len(flat)
